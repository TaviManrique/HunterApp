...
class FakeHxHApi: HxHApi {

    private val hunters = listOf(
        Hunter(
            id = 1,
            name = "Gon Freecss",
    ...
    override suspend fun searchHunters(name: String): ApiResponse {
        val searchedHunters = findHunters(name = name)
        return ApiResponse(
            success = true,
            message = "Ok",
            hunters = searchedHunters
        )
    }

    private fun findHunters(name: String): List<Hunter> {
        val founded = mutableListOf<Hunter>()
        return if (name.isNotEmpty()) {
            hunters.forEach { hunter ->
                if (hunter.name.lowercase().contains(name.lowercase())) {
                    founded.add(hunter)
                }
            }
            founded
